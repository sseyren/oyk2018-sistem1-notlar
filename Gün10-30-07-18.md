...
- `$ lspcu` -> işlemci hakkında bilgi verir
- `$ cat /proc/cpuinfo`
- `$ free -h` -> ram kullanımı hakkında bilgi verir
- `$ cat /proc/meminfo` ->
- `$ fdsik -l` -> diskler hakkında bilgileri döker
- `$ uname` -> çekirdek sürümü
- `$ cat /etc/issue` -> çekirdek sürümü
- `$ cat /etc/os-release`-> işletim sistemi hakkında genel bilgiler
- `$ uptime` -> sistemin ne kadar süredir çalıştığını gösterir
- `$ lspci` -> takılı olan PCI donanımlarını listeler
- `$ vmstat` -> 
